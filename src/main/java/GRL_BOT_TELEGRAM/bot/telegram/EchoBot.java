...
import GRL_BOT_TELEGRAM.bot.telegram.dadosbot.DadosBot;
import GRL_BOT_TELEGRAM.bot.telegram.interfacesbot.UrlCheck;
...
public class EchoBot extends TelegramLongPollingBot {
    ...
    private SendMessage responder(Update update) {
        ...
        if ("data".equals(textoMensagem)) {
            ...
            resposta = getHora();
        } else if (textoMensagem.startsWith("url")) {
            // Extrai a URL da mensagem
            String url = textoMensagem.substring(4).trim();
            // Verifica se a URL não está vazia
            if (!url.isEmpty()) {
                resposta = UrlCheck.checkURL(url);
            } else {
                resposta = "Por favor, forneça uma URL após o comando 'url'.";
            }
